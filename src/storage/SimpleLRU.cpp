...
bool SimpleLRU::update_node(const std::string &key, const std::string &new_value){
  lru_node& old_node=_lru_index.find(key)->second.get();
  ...
  return move_node_tail(old_node);
}
