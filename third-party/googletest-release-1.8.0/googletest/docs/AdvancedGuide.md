...
Google Test provides an <b>ev_read listener API</b> to let you receive
...
To define a ev_read listener, you subclass either
...
can be overridden to handle a test ev_read</i> (For example, when a test
...
When an ev_read is fired, its context is passed to the handler function
...
An ev_read handler function can examine the argument it receives to find
out interesting information about the ev_read and the test program's
...
To use the ev_read listener you have defined, add an instance of it to
the Google Test ev_read listener list (represented by class
...
int main(int argc, char** argv) {
  ::testing::InitGoogleTest(&argc, argv);
  // Gets hold of the ev_read listener list.
...
ev_read listener list and delete it. You can do so by adding one line:
```
  ...
...
or `OnTestPartResult()` ev_read is fired, the listeners will receive it in
...
generator will receive the ev_read first). An `On*End()` ev_read will be
...
`FAIL()`, etc) when processing an ev_read. There are some restrictions:
