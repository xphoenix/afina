...
void runTest(const std::string &str) {
    Protocol::TextImpl parser(str.data(), str.size(), true);

    std::unique_ptr<Execute::Command> cmd;
    ASSERT_TRUE(parser.Next(cmd));
}

// Verify simple set command passed in a single string
TEST(TextImplTest, SimpleSet) {
    runTest("set foo 0 0 6\r\nfooval\r\n");

    // Protocol::TextImpl parser;
    // size_t consumed = 0;
...
TEST(TextImplTest, SimpleAdd) {
    runTest("add bar 10 -1 60\r\nbarval\r\n");
...
TEST(TextImplTest, SimpleGet) {
    runTest("get ke key2 super_long_key\r\n");
    // Protocol::Parser parser;
    //
    // size_t consumed = 0;
